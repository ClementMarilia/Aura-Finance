...
export default function Groups() {
  ...
  const addMember = async (gid) => {
    ...
    try {
      ...
      toast.success(tr("Membro adicionado"));
  ...
  const submitEdit = async (e) => {
    ...
    try {
      ...
      toast.success(tr("Grupo atualizado")); setEditing(null); load();
    } catch (err) { toast.error(formatApiError(err)); }
  };
  const removeMember = async (gid, uid, name) => {
    if (!window.confirm(tr("Remover {name} do grupo?", { name }))) return;
  ...
  const remove = async () => {
    ...
    try {
      ...
      setConfirmDel(null);
      toast.success(tr("Grupo excluído"));
      load();
    } catch (err) {
      setConfirmDel(null);
  ...
  return (
    <div className="space-y-6" data-testid="groups-page">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight" style={{ fontFamily: "Outfit" }}>{tr("Grupos")}</h1>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button data-testid="new-group-button" className="bg-[#D96C5B] hover:bg-[#C25848] text-white rounded-xl">
              <Plus size={16} className="mr-1" /> {tr("Novo grupo")}
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>{tr("Novo grupo")}</DialogTitle></DialogHeader>
            <form onSubmit={submit} className="space-y-3">
              ...
              <div><Label>{tr("Membros (e-mails separados por vírgula)")}</Label>
              ...
              <Button type="submit" className="w-full bg-[#061B4A] hover:bg-[#1268F4] rounded-xl" data-testid="group-submit-button">{tr("Criar")}</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {list.length === 0 && <div className="card-soft text-center text-[#6B7068] md:col-span-2">{tr("Nenhum grupo")}</div>}
        {list.map(g => {
                ...
                <div>
                  ...
                  {!isOpen && (
                    <div className="text-xs text-[#6B7068] mt-1 flex items-center gap-1" data-testid={`group-summary-${g.id}`}>
                      ...
                      <span className="ml-1">{g.members.length} {tr("membro(s)")}</span>
                    </div>
                  )}
                </div>
              </button>
      ...
      <Dialog open={!!editing} onOpenChange={(v) => !v && setEditing(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>{tr("Editar grupo")}</DialogTitle></DialogHeader>
          <form onSubmit={submitEdit} className="space-y-3">
            ...
            <Button type="submit" className="w-full bg-[#061B4A] hover:bg-[#1268F4] rounded-xl" data-testid="group-edit-submit">{tr("Salvar alterações")}</Button>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <ConfirmDialog
        ...
        title={tr("Excluir grupo?")}
        description={confirmDel ? tr("\"{name}\" será removido. As despesas vinculadas a este grupo permanecem visíveis aos participantes.", { name: confirmDel.name }) : ""}
        onConfirm={remove}
